...
$(document).ready(function(){
        ...
        chart: {
            ...
            events: {
                click(event, chartContext, config ) {
                    $.ajax({
                        url:'{{ route("peng.ajax.detail.trend.audit") }}',
                        type: "GET",
                        data: {
    ...
    $('input[name="filter_tahun"]').change(function() {
        ...
        $.ajax({
            url:'{{ route("peng.ajax.trend.audit") }}',
            type: "GET",
            data: {
                dataTahun: dataTahun
            },
...
function _pieChart(data_id){
    ...
    var url = '{{ route("ajax.pie.cart", ":data_id") }}';
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                if((lakiLaki == 0) && (perempuan == 0)){
                    ...
                    if(response.dataJenisSurvey == 4){
                        ...
                        for (let x = 0; x < response.angkatan.length; x++) {
                            ...
                            var response_data;
                            $.ajax({
                                url: "{{ route('peng.ajax.get.persentase.angkatan') }}",
                                type: "GET",
                                async: false,
                                data: {
                        ...
                        for (let x = 0; x < response.prodi.length; x++) {
                            ...
                            var response_prodi;
                            $.ajax({
                                url: "{{ route('peng.ajax.get.persentase.prodi') }}",
                                type: "GET",
                                async: false,
                                data: {
                                    idNamaSurvey: data_id,
                ...
                $.each(response.hasil_survey_1_4, function(key,value){
					...
					var $key = key + 1;
                    var urlp = '{{ route("peng.ajax.jml.pertanyaan.1.4", ":data_id") }}';
                ...
                $.each(response.hasil_survey_ya_tidak, function(key,value){
                    ...
                    var urlq = '{{ route("peng.ajax.jml.ya.yidak", ":data_id") }}';
