@extends('layouts.pengelola')
@section('konten')

@section('title')
Pengelola Jenis Akreditasi SPM
@stop

<!-- start page title -->
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">Data Jenis Akrditasi</h4>
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="javascript: void(0);"><i class="ri-home-3-fill"></i></a></li>
                    <li class="breadcrumb-item"><a href="javascript: void(0);">Masterisasi</a></li>
                    <li class="breadcrumb-item active">Data Jenis Akreditasi</li>
                </ol>
            </div>
        </div>
    </div>
</div>
<!-- end page title -->
<!-- start card datatable -->
<div id="card-table" class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <div class=" d-sm-flex align-items-center justify-content-between">
                    <h4 class="card-title">Halaman Data Jenis Akreditasi</h4>
                    <button onclick="_addData()" data-bs-original-title="Tambah Data Baru" data-bs-placement="top" data-bs-toggle="tooltip" class="btn btn-info btn-sm"><i class="ri-add-fill align-middle me-1"></i> Tambah</button>
                </div>
                <hr>
                <div class="table-responsive">
                    <table class="table table-bordered datatable" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        <thead class="table-secondary">
                            <tr>
                                <th>NO</th>
                                <th>NAMA JENIS AKREDITASI</th>
                                <th>OPSI</th>
                            </tr>
                        </thead>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- end card datatable -->

@include('rules.pengelola.masterisasi.data_jenis_kegiatan.modal')
@section('js')
    @include('rules.pengelola.masterisasi.data_jenis_kegiatan.script');
@stop

@endsection