<script type="text/javascript">
	...
	var table = $('.datatable').DataTable({
		processing: true,
		serverSide: true,
		ajax: "{{ route('pengelola-data-jenis-peraturan.index') }}",
		columns: [
    ...
    function _editData(data_id) {
		...
		var url = '{{ route("pengelola-data-jenis-peraturan.edit", ":data_id") }}';
    ...
    function _deleteData(data_id) {
		Swal.fire({title: 'Apakah Kamu Yakin?',text: "Menghapus data ini?",icon: 'warning',showCancelButton:true,confirmButtonColor: '#3085d6',cancelButtonColor: '#d33',confirmButtonText:'Hapus!',cancelButtonText: "Batal",
			...
			if (result.isConfirmed) {
				var url = '{{ route("pengelola-data-jenis-peraturan.destroy", ":data_id") }}';
    				url = url.replace(':data_id', data_id);
				$.ajax({
					url: url,
					type: "DELETE",
	...
	$(document).ready(function(){
		$('#form-data').on('submit', function(event){
			...
			$.ajax({
				url: "{{ route('pengelola-data-jenis-peraturan.store') }}",
				method: 'POST',
