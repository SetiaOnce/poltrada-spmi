<script type="text/javascript">
	...
	var table = $('.datatable').DataTable({
        ...
        columnDefs: [
            ...
            { "width": "75%", "targets": 1, "className": "align-top" },
	...
	function _addData() {
		...
		$('[name="id"]').val(''),
        $('#headeTitle').html(''),
        $('#headeTitle').append('Input Jenis Kegiatan Baru');
    }
    ...
    function _editData(data_id) {
        ...
        $('#headeTitle').html('');
        $('#headeTitle').append('Edit Data Jenis Kegiatan');
		$('#modalAdd').modal('show');

		var url = '{{ route("data-jenis-kegiatan.edit", ":data_id") }}';
		...
		$.ajax({
                ...
                success:function(response){
					$('#nama_jenis_kegiatan').val(response.nama_jenis_kegiatan);
                }
    ...
    function _deleteData(data_id) {
		Swal.fire({title: 'Apakah Kamu Yakin?',text: "Menghapus data ini?",icon: 'warning',showCancelButton:true,confirmButtonColor: '#3085d6',cancelButtonColor: '#d33',confirmButtonText:'Hapus!',cancelButtonText: "Batal",
			...
			if (result.isConfirmed) {
				var url = '{{ route("data-jenis-kegiatan.destroy", ":data_id") }}';
    				url = url.replace(':data_id', data_id);
				$.ajax({
					url: url,
					type: "DELETE",
	...
	$(document).ready(function(){
		$('#form-data').on('submit', function(event){
			...
			const nama_jenis_kegiatan = $('#nama_jenis_kegiatan');     
			if (nama_jenis_kegiatan.val() == '') {
				Swal.fire({ 
					icon: 'warning',
					title: 'Maaf!',
					text: 'Nama jenis kegiatan harus diisi terlebih dahulu. terlihat saat ini masih kosong...',
					allowOutsideClick: false, 
				})     
				nama_jenis_kegiatan.focus();
