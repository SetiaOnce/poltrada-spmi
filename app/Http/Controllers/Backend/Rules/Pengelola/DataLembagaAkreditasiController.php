...
class DataLembagaAkreditasiController extends Controller
{
    protected $base = 'rules.pengelola.masterisasi.data_lembaga_akreditasi.';
    
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = DataLembagaAkreditasi::latest()->get();
    ...
    public function store(Request $request)
    {
        ...
        if($method == 'add'){
            DataLembagaAkreditasi::create([
                'nama_lembaga' => $nama_lembaga,
                'alamat' => $alamat,
                'created_at' => Carbon::now()
            ]);
        }else if($method == 'update'){
            DataLembagaAkreditasi::find($id)->update([
                'nama_lembaga' => $nama_lembaga,
                'alamat' => $alamat,
    ...
    {
        $data = DataLembagaAkreditasi::find($id);
        return response()->json($data);
    }

    public function destroy($id)
    {
        DataLembagaAkreditasi::find($id)->delete();
